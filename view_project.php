		...
		<div class="col-md-4">
			<div class="card card-outline card-primary">
				<div class="card-header">
					...
					<button class="btn btn-primary bg-gradient-primary btn-sm" type="button" id="manage_team" data-project-id="<?php //echo $_GET['id']; ?>">Manage</button>
			...
			<div class="card card-outline card-primary">
				<div class="card-header">
					...
					<div class="card-tools">
						...
						<button class="btn btn-primary bg-gradient-primary btn-sm" type="button" id="deligations"><i class="fa fa-plus"></i> Deligations</button>
					</div>
				<?php endif; ?>
				</div>
	...
	<div class="table-responsive">
		<table class="table table-condensed m-0 table-hover">
			...
			<tbody>
				<?php 
				$i = 1;
				$where = ($_SESSION['login_type'] == 3) ? "AND t.user_id = {$_SESSION['login_id']}" : "";
				$tasks = $conn->query("SELECT t.*, u.lastname FROM task_list t INNER JOIN users u ON t.user_id = u.id WHERE t.project_id = {$id} $where ORDER BY t.task ASC");
...
<div class="modal fade" id="manageTeamModal" tabindex="-1" role="dialog" aria-labelledby="manageTeamModalLabel" aria-hidden="true">
...
$(document).ready(function(){
    $('#manage_team').click(function(){
        ...
        $.ajax({
            url: 'edit_team.php',
            method: 'GET',
            data: { id: <?php echo $id; ?> }, // Pass the project id to edit_project.php
            success: function(response){
                $('#manageTeamContent').html(response);
    ...
    // Existing functions for managing tasks and productivity
    $('#new_task').click(function(){
        uni_modal("New Task For <?php echo ucwords($name) ?>","manage_task.php?pid=<?php echo $id ?>","mid-large");
    });

    $('#deligations').click(function(){
        uni_modal("New Task For <?php echo ucwords($name) ?>","deligations.php?pid=<?php echo $id ?>","mid-large");
    });
